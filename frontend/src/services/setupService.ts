...
export const setupService = {
  getProgress: async (): Promise<SetupProgressResponse> => {
    const response = await apiClient.get<SetupProgressResponse>('/api/onboarding/progress');
  ...
  submitStep2: async (data: { trade_duration: string; state: string; lga: string; income_range: string }): Promise<SetupStepResponse> => {
    const response = await apiClient.post<SetupStepResponse>('/api/onboarding/step2', data);
    return response.data;
  },
  submitStep3: async (data: { saves_money: boolean; savings_methods: string[]; in_ajo_group: boolean; contribution_consistency: string }): Promise<SetupStepResponse> => {
    const response = await apiClient.post<SetupStepResponse>('/api/onboarding/step3', data);
    return response.data;
  },
  submitStep4: async (data: { has_borrowed: boolean; repaid_fully: boolean; repaid_on_time: boolean }): Promise<SetupStepResponse> => {
    const response = await apiClient.post<SetupStepResponse>('/api/onboarding/step4', data);
    return response.data;
  },
  submitStep5: async (data: { language: string; profile_photo: string | null }): Promise<SetupFinalResponse> => {
